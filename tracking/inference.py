...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # CASE "jail" edge case noisyDistance is None
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # iterate over old pos newpos and prob
        # new postion depends on prob of finding at t+1 position and beliefs at old position
        for pos in self.legalPositions:
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for lpos in self.legalPositions:
                trueDistance = util.manhattanDistance(lpos, pacmanPosition)
                allPossible[lpos] += emissionModel[trueDistance] * bdist[lpos]
            # **** second special case ****
            allPossible.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numParticles -1):
            self.particles = [permutations for permutations in permutationsOfLegalPositions if i < self.numParticles]
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            posState[particle] += probability


        if posState.totalCount() == 0:
            return self.initializeParticles()
            # second special case
        posState.normalize()
        distance = [util.sample(posState) for x in range(self.numParticles)]
        self.particles = distance
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            while i < self.numGhosts:
                posDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i,
                                                             self.ghostAgents[i])
